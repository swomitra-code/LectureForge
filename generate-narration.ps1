...
foreach ($slide in $requestedSlides) {
    ...
    for ($takeNumber = 1; $takeNumber -le 3; $takeNumber++) {
        ...
        try {
            ...
            if ($null -ne $status) { throw "ElevenLabs generation failed for Slide $slideNumber Take $takeNumber (HTTP $status)." }
            throw "ElevenLabs generation failed for Slide $slideNumber Take $takeNumber."
        }
