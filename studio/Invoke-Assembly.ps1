...
function Progress($Stage,$Slide){Invoke-LFNarrationLock $RuntimeRoot $Id {param($d) $a=Read-LFAssembly $d;if($a.current.id -ne $Attempt){throw 'Assembly attempt replaced.'};$a.current.stage=$Stage;$a.current.slide=$Slide;Write-LFJson (Join-Path $d 'assembly.json') $a}}
try{
 ...
 Invoke-LFNarrationLock $RuntimeRoot $Id {param($d)
  ...
  $a.current.output=$dest;$a.current.output_sha256=$result.output_sha256;$a.current.stage='Complete';$a.current.lease=$null
 ...
 Invoke-LFNarrationLock $RuntimeRoot $Id {param($d) $a=Read-LFAssembly $d;if($a.current.id -eq $Attempt){$a.current.error=$message;$a.current.stage='Exception';$a.current.lease=$null;Write-LFJson (Join-Path $d 'assembly.json') $a}}
 exit 1
