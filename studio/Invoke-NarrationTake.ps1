...
$context=Invoke-LFNarrationLock $RuntimeRoot $Id {param($dir)
 $state=Read-LFNarrationFile $dir;$r=@($state.revisions|Where-Object id -eq $Revision)[0];$t=@($r.takes|Where-Object number -eq $Take)|Select-Object -First 1;$a=$t.attempts[-1]
...
try{
 ...
 $diagnostic=Get-NarrationFailureDiagnostic $_ @($env:ELEVENLABS_API_KEY,$env:HEYGEN_API_KEY,$context.revision.script,(ConvertTo-LFSpokenText $context.revision.script))
